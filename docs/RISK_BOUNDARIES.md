...
- **`client_escrow_pact_v4.json`** - Client `contract_call` allowlist: `target_in` = **escrow v4
  (`0x8F60…264C`) + MockUSDC only**, plus a per-24h tx-count cap. The client can call createJob / approve /
  fund / finalize - and nothing else, nowhere else.
...
`pacts.provider_pact` / `client_escrow_pact` bound to escrow v4). So whatever LLM connects, the provider Pact
